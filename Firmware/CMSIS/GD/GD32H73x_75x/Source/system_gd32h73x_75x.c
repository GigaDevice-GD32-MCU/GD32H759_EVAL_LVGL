...
Note: the power mode need to match the mcu selection and external power supply circuit.
    for iar project:
        for 100-pin mcu, need to define macro GD32H73XV_75XV.
        for 144-pin mcu, need to define macro GD32H73XZ_75XZ.
        for 176-pin mcu, need to define macro GD32H73XI_75XI.
    for keil project:
...
#elif defined(GD32H73XZ_75XZ) | defined(GD32H73XV_75XV)
//#define SEL_PMU_SMPS_MODE   PMU_LDO_SUPPLY
//#define SEL_PMU_SMPS_MODE   PMU_BYPASS
#endif
#else
...
#else
    RCU_CFG2 &= ~(RCU_CFG2_CKOUT0SEL | RCU_CFG2_CKOUT1SEL | RCU_CFG2_CKOUT0DIV | RCU_CFG2_CKOUT1DIV);
#endif /* GD32H75E */
#ifndef GD32H75E
...
#ifdef GD32H75E_ESC
/*!
    \brief      disable oe_ext clock output
    \param[in]  none
    \param[out] none
...
static ErrStatus esc_oeext_clock_disable(void)
{
    ...
    ErrStatus error_status = SUCCESS;
    ...
    do {
        /* byte read test and PMU module ready */
        ospi_read(PMU_PDIREFVAL, (uint8_t *)&temp, 4U);
        timeout--;
    } while((0x76543210U != temp) && (0U != timeout));

    if(0U == timeout) {
        /* configuration failed, please check the hardware */
        error_status = ERROR;
    }

    ospi_read(ESC_SYSCFG_RESERVED, (uint8_t *)&temp, 4U);
    temp |= 0x00000001U;
    ospi_write(ESC_SYSCFG_RESERVED, (uint8_t *)&temp, 4U);

    timeout = 0xFF;
    do {
        /* check OEEXTCFG bit whether enabled */
        ospi_read(ESC_SYSCFG_RESERVED, (uint8_t *)&temp, 4U);
        timeout--;
    } while((0x00000001U != (temp & 0x00000001U)) && (0U != timeout));
...
*/
uint32_t gd32h73x_75x_firmware_version_get(void)
{
    return __GD32H73X_75X_STDPERIPH_VERSION;
}
