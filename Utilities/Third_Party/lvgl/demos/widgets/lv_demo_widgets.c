...
/* Timer created in analytics_create; keep handle to safely delete */
static lv_timer_t * meter2_timer = NULL;
...
static void home_event_cb(lv_event_t *e)
{
    ...
    /* Stop timer and running animations to avoid callbacks on deleted objects */
    if(meter2_timer) {
...
void lv_demo_widgets(void)
{
    ...
    lv_obj_t *imgbtn_home = lv_imgbtn_create(tv);
    lv_imgbtn_set_src(imgbtn_home, LV_IMGBTN_STATE_RELEASED, &img_home, NULL, NULL);
    lv_obj_set_size(imgbtn_home, 30, 30);
    lv_obj_set_pos(imgbtn_home, 879, 435);
    lv_obj_add_event_cb(imgbtn_home, home_event_cb, LV_EVENT_CLICKED, tv);

    lv_obj_set_style_text_font(tv, font_normal, 0); //parent
...
static void profile_create(lv_obj_t * parent)
{
    ...
    /*Create a keyboard*/
    lv_obj_t * kb = lv_keyboard_create(lv_scr_act());  //parent
...
static void color_changer_create(lv_obj_t * parent)
{
    ...
    if(disp_size == DISP_SMALL) {
        ...
        lv_obj_set_size(btn, LV_DPX(30), LV_DPX(30));
        lv_obj_align(btn, LV_ALIGN_BOTTOM_RIGHT, -LV_DPX(15), -LV_DPX(15));
    }
}
