...
static bool benchmark_running = false;
static finished_cb_t * benchmark_finished_cb = NULL;
static finished_cb_t * benchmark_exit_cb = NULL;
...
static void home_event_cb(lv_event_t *e)
{
    ...
    lv_disp_get_default()->driver->monitor_cb = NULL;

    /* Restore the refresh/animation periods if they were forced to max speed. */
    if(run_max_speed) {
        lv_timer_t * anim_timer = lv_anim_get_timer();
        if(anim_timer != NULL && anim_ori_timer_period != 0) {
            lv_timer_set_period(anim_timer, anim_ori_timer_period);
        }
        lv_disp_t * disp = lv_disp_get_default();
        lv_timer_t * refr_timer = _lv_disp_get_refr_timer(disp);
        if(refr_timer != NULL && disp_ori_timer_period != 0) {
            lv_timer_set_period(refr_timer, disp_ori_timer_period);
        }
    }

    /* benchmark_init() stripped every style from the active screen and
     * generate_report() additionally puts a flex layout on it. Leaving that
     * behind makes the page rebuilt by the exit callback flicker and lay out
     * incorrectly, so put the screen back into its default (themed) state. */
...
static void create_home_button(lv_obj_t * parent)
{
    imgbtn_home = lv_imgbtn_create(parent);
    /* img_home_pressed is a 60x60 asset while img_home is 30x30. lv_imgbtn
     * draws the source at its native size, so using the pressed asset made the
     * button visibly double in size while touched. Use the same 30x30 image
     * for both states. */
    ...
    /* The report page uses a flex layout on the screen; keep the exit button
     * out of it so it always stays in the top right corner. */
    lv_obj_add_flag(imgbtn_home, LV_OBJ_FLAG_IGNORE_LAYOUT);
    lv_obj_align(imgbtn_home, LV_ALIGN_TOP_RIGHT, -3, 3);
    lv_obj_move_foreground(imgbtn_home);
...
void lv_demo_benchmark_set_exit_cb(finished_cb_t * exit_cb)
{
    benchmark_exit_cb = exit_cb;
}
...
static void generate_report(void)
{
    ...
    lv_obj_set_style_min_height(table, 28, LV_PART_ITEMS);
    lv_obj_set_style_max_height(table, 28, LV_PART_ITEMS);

    create_home_button(lv_scr_act());
